...
= Part 4 - Events (Updated with Docker and Mysql... so far... stay tuned!)
:sourcedir: https://github.com/jamesphilipreilly/dockerdbjava



In the <<react-and-spring-data-rest-part-4, section>>, you learnws how to use Spring Data REST's built in event system to detect changes in the backend and publish updates to ALL users through Spring's WebSocket support. Then you'll be able to dynamically adjust clients as the data updates.

Feel free to {sourcedir}/events[grab the code] from this repository. This section is based on the previous section with several twists.

== Spring WebSocket is Already Added... Now we are updating the databases and containers
